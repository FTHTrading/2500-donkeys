...
contract PublishingKernel {
    ...
    struct Edition {
        ...
        // AI provenance
        string  aiModel;          // e.g., "stable-diffusion-xl" or "" if none
        bytes32 promptSetHash;    // hash of the prompt configuration used
    ...
    struct License {
        ...
        address royaltyRouter;    // address of associated RoyaltyRouter, or address(0)
        bool    revoked;
    }
    ...
    string  public title;
    address public immutable genesisAnchor; // address of the original LiteraryAnchor

    Edition[] public editions;
    License[] public licenses;

    // Mappings for efficient lookups
    mapping(uint256 => uint256[]) public editionLicenses; // editionId => license indices
    mapping(bytes32 => bool) public anchoredRoots;        // editionRoot => anchored?
    ...
    /**
     * @notice Retract an edition with a reason hash.
     */
    function retract(
        uint256 _editionId,
    ...
    /**
     * @notice Restore canonicality to an edition (e.g., after review).
     */
